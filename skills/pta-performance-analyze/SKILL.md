...
Config: [servers.json](servers.json). The script runs NPU and GPU tests remotely, collects memory JSON and (for NPU) filtered plog, and GPU operator-trace log; all outputs are written under the NPU script’s directory.
...
- `gpu_mem_trace_<api_name>.log` — GPU operator-level memory log (optional; format aligned with filtered plog for Agent comparison)
...
4. **GPU operator trace** (`gpu_mem_trace_*.log`) — **[Optional]** If present, same structure as plog Summary: `#N: ... bytes ... | op: aten::...`. Compare which GPU ops use how much self memory vs NPU’s Workspace allocs per op.
