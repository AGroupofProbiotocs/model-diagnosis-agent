...
3. **Determine fix location:**
   ...
   - **CANN issue** (workspace too large, internal Cast) → describe workspace/cast behavior, recommend filing DTS or feature request to CANN team

**Note:** Root cause must be reported for ALL cases. If overconsumption is NOT from `TARGET_API` (identified in Step 1), still report root cause but clarify that the issue is in auxiliary ops, not the target API being tested.
