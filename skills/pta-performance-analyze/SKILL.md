...
- User provides path → use directly
- User doesn't provide → scan for `torchapi_*.py` files
  - Exactly one match → use it
  - Multiple matches → ask user to select
  - None found → ask user for path
- Script must contain `TARGET_API = "torch.xxx"` and `memory_decorator`
...
**Do not ask the user in this stage.** Reuse existing artifacts if available:
- `mem_results.md` + `filtered_plog_*.log` exist → proceed to Stage 3
- Otherwise → run benchmarks using `run_remote_mem_test.py <npu_script> <gpu_script>`

Outputs (in NPU script directory):
- `mem_results.md` — NPU vs GPU comparison (required)
- `filtered_plog_<api>.log` — NPU memory trace (required)
- `gpu_mem_trace_<api>.log` — GPU trace (optional)
...
> The GPU operator trace log is **optional**. Absence of `gpu_mem_trace_*.log` does NOT block Stage 3.

## Stage 3: Analyze Root Cause

### Step 1: Analyze Memory and Locate NPU Overconsumption (Preliminary)

Use the following inputs to locate overconsumption:

1. **Test scenario** (mem_results.md Key Code) — input shape/dtype, estimate baseline
2. **Memory comparison** (mem_results.md) — NPU vs GPU ratio
3. **NPU plog** (`filtered_plog_*.log`) — [Summary] Workspace allocs, peak malloc
4. **GPU trace** (`gpu_mem_trace_*.log`) — **[Optional]** aten:: ops memory

**Analysis goal:** From (1)–(4), pinpoint **which NPU-side operators or stages** account for the extra memory (e.g. a specific aclnn op’s workspace much larger than GPU’s equivalent, or extra Cast/Contiguous/Nonzero). Output a short list: suspected aclnn interface + estimated impact.
...
> **Prerequisite**: If user provided `pytorch_npu` source path earlier, use it. Otherwise ask: "Please provide your pytorch_npu repository path (e.g. `d:\open_source\pytorch_npu`)." If unavailable → skip to conclusions (plog + known issues only).

If source available, proceed:
...
### Step 4: Present Findings (→ Stage 4 if user confirms resolved)

Collect ALL conclusions and report to user:
...
```
[User reports NPU memory issue]
    │
    ▼
Stage 1: Find NPU script → Generate GPU script
    │
    ▼
Stage 2: Run benchmarks → Check ratio
    │
    ├── ratio ≤ 1.05 ──→ [STOP] Tell user: "Normal, verify test script"
    │
    └── ratio > 1.05 ──→ Stage 3: Analyze root cause
                              │
                              ▼
                         Step 1: Plog analysis
                              │
                              ▼
                         Step 2: Match known issues
                              │
                              ├── Found match → Close loop
                              │
                              └── No match → Step 3: Source analysis
                                                   │
                                                   ▼
                                            Present findings
                                                   │
                                                   ▼
Stage 4: User confirms → Accumulate experience
```
...
- [capture_gpu_mem_trace.py](tools/capture_gpu_mem_trace.py) — GPU operator-level memory log (optional; integrated in run_remote_mem_test.py)
