...
CAPTURE_GPU_SCRIPT = SCRIPT_DIR / "capture_gpu_mem_trace.py"
...
def run_gpu_test(cfg, gpu_script, api_name, out_dir, results):
    ...
    try:
        ...
        # 可选：抓取 GPU 算子级显存 log（与 NPU filtered_plog 可对比），失败不阻塞
        if json_result and CAPTURE_GPU_SCRIPT.is_file():
            try:
                log(T, "抓取 GPU 算子级显存 log ...")
                cap_name = CAPTURE_GPU_SCRIPT.name
                ok_cap, err_cap = scp_upload(
                    target, str(CAPTURE_GPU_SCRIPT), f"{wdir}/{cap_name}", env
                )
                if not ok_cap:
                    log(T, f"上传 {cap_name} 失败（跳过 GPU trace）: {err_cap}")
                else:
                    use_32 = "--32align" if json_result.get("32aligned") else ""
                    api_safe = (api_name or "unknown").replace(".", "_")
                    remote_trace = f"{wdir}/gpu_mem_trace_{api_safe}.log"
                    trace_cmd = (
                        f"cd {wdir} && "
                        f"{{ [ -f {base}/env.sh ] && source {base}/env.sh; true; }} && "
                        f"python {cap_name} {sn} -o gpu_mem_trace_{api_safe}.log {use_32} 2>&1"
                    )
                    trace_out, trace_err, trace_rc = ssh_run(
                        target, trace_cmd, env, timeout=300
                    )
                    if trace_rc == 0:
                        local_trace = os.path.join(out_dir, f"gpu_mem_trace_{api_safe}.log")
                        ok_dl, err_dl = scp_download(
                            target, remote_trace, local_trace, env
                        )
                        if ok_dl:
                            results["gpu_trace_log"] = local_trace
                            log(T, f"GPU trace 已下载: {local_trace}")
                        else:
                            log(T, f"下载 GPU trace 失败: {err_dl}")
                    else:
                        log(T, f"GPU trace 执行失败（跳过）: {trace_out[-400:]}")
            except Exception as e_cap:
                log(T, f"GPU trace 异常（跳过）: {e_cap}")
...
def write_results(results, path, key_code=None, script_path=None, line_range=None):
    ...
    if key_code:
        L.append("## Key Code")
        if script_path and line_range:
            script_name = os.path.basename(script_path)
            L.append(f"Source: [{script_name}:L{line_range[0]}-L{line_range[1]}]({script_path}#L{line_range[0]}-L{line_range[1]})")
        L.append("```python")
        L.append(key_code)
        L.append("```\n")
    ...
    if results.get("gpu_trace_log"):
        L += [
            "## GPU operator trace (optional)",
            f"- gpu_mem_trace: {results['gpu_trace_log']}",
            "",
        ]
